...
type PakeProtocol struct {
	...
	stepEmitter   *events.Emitter[pakeEvent[PakeStep]]
	onKeyExchange *events.Emitter[pakeOnKeyExchange]
}
...
// the PAKE protocol.
func NewPakeProtocol(ctx context.Context, host host.Host, words []string) *PakeProtocol {
	return &PakeProtocol{
		ctx:         ctx,
		host:        host,
		pwKey:       []byte(strings.Join(words, "-")),
		states:      map[peer.ID]*PakeState{},
		stepEmitter: events.NewEmitter[pakeEvent[PakeStep]](),
	}
}
...
func (p *PakeProtocol) Update(msg tea.Msg) (*PakeProtocol, tea.Cmd) {
	...
	case events.Msg[pakeEvent[PakeStep]]:
		defer close(msg.Value.handled)
		evt := msg.Value
		...
		return p, msg.Listen
	case events.Msg[pakeOnKeyExchange]:
		remotePeer := msg.Value.Conn().RemotePeer()
		...
		if found {
			...
			default:
				...
				return p, p.onKeyExchange.Subscribe
			}
		}
...
func (p *PakeProtocol) RegisterKeyExchangeHandler() tea.Cmd {
	log.Infoln("Registering key exchange handler")
	p.onKeyExchange = events.NewEmitter[pakeOnKeyExchange]()
	p.host.SetStreamHandler(ProtocolPake, func(s network.Stream) {
		p.onKeyExchange.Emit(pakeOnKeyExchange(s))
	})

	return tea.Batch(
		p.stepEmitter.Subscribe,
		p.onKeyExchange.Subscribe,
	)
}

func (p *PakeProtocol) UnregisterKeyExchangeHandler() {
	log.Infoln("Unregistering key exchange handler")
	p.onKeyExchange.Close()
	p.host.RemoveStreamHandler(ProtocolPake)
}

type pakeEvent[T any] struct {
	peerID  peer.ID
	payload T
	handled chan struct{}
}
...
func (p *PakeProtocol) exchangeKeys(s network.Stream) tea.Cmd {
	return func() tea.Msg {
		...
		if err != nil {
			return pakeEvent[error]{peerID: remotePeer, payload: err}
		}

		// Waiting for key information...
		p.emitPakeStep(remotePeer, PakeStepWaitingForKeyInformation)
		...
		p.emitPakeStep(remotePeer, PakeStepCalculatingKeyInformation)
		...
		p.emitPakeStep(remotePeer, PakeStepSendingKeyInformation)
		...
		p.emitPakeStep(remotePeer, PakeStepWaitingForFinalKeyInformation)
		...
		p.emitPakeStep(remotePeer, PakeStepExchangingSalt)

		// Generating session key salt
		salt := make([]byte, 8)
		if _, err = rand.Read(salt); err != nil {
			return pakeEvent[error]{peerID: remotePeer, payload: fmt.Errorf("read randomness: %w", err)}
		}
		...
		p.emitPakeStep(remotePeer, PakeStepProvingAuthenticityToPeer)
		...
		if err = p.SendProof(s, key); err != nil {
			return pakeEvent[error]{peerID: remotePeer, payload: fmt.Errorf("send proof: %w", err)}
		}

		// Verifying proof from peer...
		p.emitPakeStep(remotePeer, PakeStepVerifyingProofFromPeer)

		// Read and verify encryption proof from P
		if err = p.ReceiveVerifyProof(s, key); err != nil {
			return pakeEvent[error]{peerID: remotePeer, payload: fmt.Errorf("receive verify proof: %w", err)}
		}
		...
		return pakeEvent[[]byte]{peerID: remotePeer, payload: key}
	}
}
...
func (p *PakeProtocol) StartKeyExchange(ctx context.Context, remotePeer peer.ID) tea.Cmd {
	return func() tea.Msg {
		...
		// Authenticating peer...
		p.emitPakeStep(remotePeer, PakeStepStart)
		...
		key, err := crypt.DeriveKey(skey, salt)
		if err != nil {
			return pakeEvent[error]{peerID: remotePeer, payload: fmt.Errorf("derive key from session key and salt: %w", err)}
		}
		...
		p.emitPakeStep(remotePeer, PakeStepWaitingForFinalConfirmation)
		...
		if string(confirm) != "ok" {
			return pakeEvent[error]{peerID: remotePeer, payload: fmt.Errorf("peer did not respond with ok")}
		}
...
func (p *PakeProtocol) emitPakeStep(peerID peer.ID, s PakeStep) {
	evt := pakeEvent[PakeStep]{
		peerID:  peerID,
		payload: s,
		handled: make(chan struct{}),
	}
	p.stepEmitter.Emit(evt)
	<-evt.handled
}
